...
class PostController {
  ...
  update = async (request: Request, response: Response) => {
    ...
    try {
      ...
      if (decodedToken.data.id !== post.user_id) {
        return response.status(400).json({ message: 'This post is not your, so you cannot update.' });
      }
      await knex('posts')
        .where('id', '=', id)
        .update({ message });
  ...
  delete = async (request: Request, response: Response) => {
    ...
    try {
      ...
      return response.status(400).json({ message: 'This post is not yours, so you cannot delete it.' });
    } catch (error) {
      return response.status(400).json({ message: 'Invalid token' });
    }
  }
