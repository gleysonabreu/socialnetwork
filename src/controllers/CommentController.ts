...
class CommentController {
  create = async (request: Request, response: Response) => {
    ...
    try {
      ...
      if (!post) {
        return response.status(400).json({ message: 'Post not found, so you cannot comment it.' });
      }
  ...
  update = async (request: Request, response: Response) => {
    ...
    try {
      ...
      if (comment.user_id !== decodedToken.data.id) {
        return response.status(400).json({ message: 'This comment is not yours, so you cannot update it.' });
      }
  ...
  delete = async (request: Request, response: Response) => {
    ...
    try {
      ...
      if (commentDel.user_id !== decodedToken.data.id) {
        return response.status(400).json({ message: 'This comment is not yours, so you cannot delete it' });
      }
